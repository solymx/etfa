"""兆豐投信 fetcher（⚠️ 待實作 - 需要您抓到真實 API 後填入）

📍 已知資訊
    ETF：00996A 兆豐台灣豐收
    頁面：https://www.megafunds.com.tw/
    00996A 是兆豐首檔主動式 ETF
...
# TODO ① 調查到 API 後，填入這裡
# ============================================================================
API_URL = ""  # 例：https://api.megafunds.com.tw/fund/holdings
METHOD = "GET"  # 或 "POST"
...
_HEADERS = {
    ...
    "Referer": "https://www.megafunds.com.tw/",
...
def fetch(fund_code: str, **_) -> pd.DataFrame:
    ...
    Args:
        fund_code: 兆豐投信內部 ETF 代號（從 etfs.yaml params.fund_code 傳入）
    ...
    if not API_URL:
        raise NotImplementedError(
            "megafund fetcher 尚未實作。請參考檔案開頭 TODO 區塊，或見 "
            "docs/PHASE2_INVESTIGATION.md 與 docs/FIND_API_SOP.md"
        )

    source = f"megafund/{fund_code}"
